...
const Leaderboard = (props) => {
  ...
  const side = (holes, sd) => {
    ...
    const rows = holes.map((h) => {
      const scrs = orderedPlayerList.map((p) => {
        ...
        const t = find(h.teams, { team: p.team });
        let team = null;
        if (t?.team) {
          team = t.team;
        } // dafuq?
  ...
  const Row = ({ row }) => {
    const scoreCells = row.scores.map((s, i) => {
      if (!s.score || !s.score[scoreType]) {
        return (
          <View
            key={`cell_${row.hole}_${i}`}
            style={styles.scorePopContainer}
          />
        );
      }
  ...
  const TotalRow = ({ section }) => {
    ...
    const totalCells = orderedPlayerList.map((p) => (
      <View
        key={`totalcell_${section.side}_${p.pkey}`}
        style={styles.scorePopContainer}
      >
        <View style={styles.scoreView}>
          <View style={birdieShape}>
  ...
  const Header = () => {
    const players = orderedPlayerList.map((p) => {
      return (
        <View
          key={`header_${p.pkey}`}
          style={[styles.playerNameView, styles.rotate]}
        >
          <Text
