...
4. **Generate commit message**:
   ...
   - Scopes: `app`, `api`, `lib`, `web`, `scoring`, `e2e` (or omit if cross-cutting)
...
5. **Commit**:
   ...
   ```
   Use 60000ms timeout — pre-commit hooks take 15-45 seconds.

6. **Report** the commit hash and summary to the user

If the user provides arguments (e.g., `/commit "fix(app): resolve race condition"`), use that as the commit message instead of generating one.
